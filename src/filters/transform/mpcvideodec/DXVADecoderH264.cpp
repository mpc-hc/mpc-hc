...
void CDXVADecoderH264::CopyBitstream(BYTE* pDXVABuffer, BYTE* pBuffer, UINT& nSize)
{
	...
	do
	{
		ReadNalu (&Nalu, pDataSlice, nSliceSize, 4);
		...
		switch (Nalu.nal_unit_type)
		{
			...
			default :
				...
				// Copy NALU
				memcpy (pDXVABuffer+4, (BYTE*)pDataSlice+4, Nalu.len-4);
...
HRESULT CDXVADecoderH264::DecodeFrame (BYTE* pDataIn, UINT nSize, REFERENCE_TIME rtStart, REFERENCE_TIME rtStop)
{
	...
	while (!bSliceFound)
	{
		...
		case NALU_TYPE_PPS :
			//ReadPPS(&m_PicParam, Nalu.data, RBSPtoSODB(Nalu.data,  Nalu.data_len));
			//UpdatePictureParams(false);

			m_pFilter->DecodeData (pDataSlice, nSliceSize);
			FFUpdatePictureParam (false, &m_DXVAPicParams, &m_DXVAScalingMatrix, m_pFilter->GetAVCtx());

			pDataSlice	+= Nalu.len;
			nSliceSize	-= Nalu.len;
			break;
		case NALU_TYPE_SPS :
			//ReadSPS(&m_SeqParam, Nalu.data, Nalu.data_len);		// TODO : buffer size correct ??
			//UpdatePictureParams(false);
	...
	UpdateRefFramesList (m_Slice.frame_num, (Nalu.nal_reference_idc != 0));
...
void CDXVADecoderH264::UpdatePictureParams(bool bInit)
{
	if (bInit)
	{
...
//	m_DXVAPicParams.CurrFieldOrderCnt						= SET IN UpdateRefFramesList;
//	m_DXVAPicParams.FieldOrderCntList						= SET IN UpdateRefFramesList;
//	m_DXVAPicParams.FrameNumList							= SET IN UpdateRefFramesList;
//	m_DXVAPicParams.UsedForReferenceFlags					= SET IN UpdateRefFramesList;
...
//	m_DXVAPicParams.SliceGroupMap
	...
	memcpy (&m_DXVAScalingMatrix, &m_PicParam.QuantMatrix, sizeof(DXVA_Qmatrix_H264));
}
...
void CDXVADecoderH264::SetExtraData (BYTE* pDataIn, UINT nSize)
{
	...
	UpdatePictureParams(true);

	DXVA_PicParams_H264		Test;
	FFUpdatePictureParam (true, &Test, &m_DXVAScalingMatrix, m_pFilter->GetAVCtx());
	int x = memcmp (&m_DXVAPicParams, &Test, sizeof(DXVA_PicParams_H264));
	ASSERT (x == 0);

	FFUpdatePictureParam (true, &m_DXVAPicParams, &m_DXVAScalingMatrix, m_pFilter->GetAVCtx());
}
...
void CDXVADecoderH264::Scaling_List(UCHAR* scalingList, int sizeOfScalingList, BYTE* pBuffer, UINT nBufferLength, UINT& nBitOffset)
{
	int j, scanj;
	...
	for(j=0; j<sizeOfScalingList; j++)
	{
		...
		if(nextScale!=0)
		{
			delta_scale = se_v (pBuffer, nBufferLength, nBitOffset);
...
//	int			chroma_format_idc;
	...
	memcpy (&pps->QuantMatrix, (void*)&g_QMatrixH264[Flat16], sizeof(DXVA_Qmatrix_H264));
	...
	if(more_rbsp_data(pBuffer, nBitOffset, nBufferLength)) // more_data_in_rbsp()
	{
		...
		if(pps->pic_scaling_matrix_present_flag)
		{
			...
			n_ScalingList	= 6 + 2 * pps->transform_8x8_mode_flag;
			...
			for(i=0; i<n_ScalingList; i++)
			{
				...
				{
					if(i<6)
						Scaling_List(pps->QuantMatrix.bScalingLists4x4[i], 16, pBuffer, nBufferLength, nBitOffset);
					else
						Scaling_List(pps->QuantMatrix.bScalingLists8x8[i-6], 64, pBuffer, nBufferLength, nBitOffset);
					//if(i<6)
