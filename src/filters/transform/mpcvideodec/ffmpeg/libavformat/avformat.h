 ...
 */
FF_EXPORT static inline void av_free_packet(AVPacket *pkt)
{
    if (pkt && pkt->destruct) {
        pkt->destruct(pkt);
    }
...
FF_EXPORT void av_register_all(void);
 ...
 */
FF_EXPORT AVInputFormat *av_find_input_format(const char *short_name);
 ...
 */
FF_EXPORT AVInputFormat *av_probe_input_format(AVProbeData *pd, int is_opened);

/**
 * Allocates all the structures needed to read an input stream.
 *        This does not open the needed codecs for decoding the stream[s].
 */
FF_EXPORT int av_open_input_stream(AVFormatContext **ic_ptr,
 ...
 * @return 0 if OK. AVERROR_xxx otherwise.
 */
FF_EXPORT int av_open_input_file(AVFormatContext **ic_ptr, const char *filename,
                       AVInputFormat *fmt,
                       int buf_size,
                       AVFormatParameters *ap);
/**
 ...
 */
FF_EXPORT AVFormatContext *av_alloc_format_context(void);
 ...
 */
FF_EXPORT int av_find_stream_info(AVFormatContext *ic);
 ...
 * @return 0 if OK. AVERROR_xxx if error.
 */
FF_EXPORT int av_read_packet(AVFormatContext *s, AVPacket *pkt);
 ...
 */
FF_EXPORT int av_read_frame(AVFormatContext *s, AVPacket *pkt);
 ...
 * @return >= 0 on success
 */
FF_EXPORT int av_seek_frame(AVFormatContext *s, int stream_index, int64_t timestamp, int flags);

/**
 * start playing a network based stream (e.g. RTSP stream) at the
 * current position
 */
FF_EXPORT int av_read_play(AVFormatContext *s);
 ...
 */
FF_EXPORT int av_read_pause(AVFormatContext *s);

/**
 * Free a AVFormatContext allocated by av_open_input_stream.
 * @param s context to free
 */
FF_EXPORT void av_close_input_stream(AVFormatContext *s);
 ...
 * @param s media file handle
 */
FF_EXPORT void av_close_input_file(AVFormatContext *s);
