...
#define PACKET_IS_SYNCPOINT      0x08
 ...
 #endif
#else
#define AV_DEBUG(...) av_log(NULL,AV_LOG_DEBUG, __VA_ARGS__)
#endif
...
static int vorbis_parse_id_hdr(vorbis_context *vc){
    ...
    static const float *vwin[8]={ vwin64, vwin128, vwin256, vwin512, vwin1024, vwin2048, vwin4096, vwin8192 };
    ...
    vc->channel_residues=(float *)av_mallocz((vc->blocksize_1/2)*vc->audio_channels * sizeof(float));
    vc->channel_floors=(float *)av_mallocz((vc->blocksize_1/2)*vc->audio_channels * sizeof(float));
    vc->saved=(float *)av_mallocz((vc->blocksize_1/2)*vc->audio_channels * sizeof(float));
    vc->ret=(float *)av_mallocz((vc->blocksize_1/2)*vc->audio_channels * sizeof(float));
...
static int vorbis_parse_comment_hdr(vorbis_context *vc) {
    ...
    vendorlen=get_bits_long(gb,32);
    if(vendorlen<0) return -1;
    vc->vendor=av_mallocz(vendorlen+1);
    vorbis_readstring(gb,vc->vendor,vendorlen);
    vc->comments=get_bits_long(gb,32);
    if(vc->comments<0) return -1;
...
static int vorbis_init_id_header(AVCodecContext *avccontext,const uint8_t *headers,int header_len)
{
    vorbis_context *vc = avccontext->priv_data ;
    GetBitContext *gb = &(vc->gb);
    int hdr_type;
    init_get_bits(gb, headers, header_len*8);
...
static int vorbis_init_setup_header(AVCodecContext *avccontext,const uint8_t *headers,int header_len)
{
    vorbis_context *vc = avccontext->priv_data ;
    GetBitContext *gb = &(vc->gb);
    int hdr_type;
...
static int vorbis_init_comment_header(AVCodecContext *avccontext,const uint8_t *headers,int header_len)
{
    vorbis_context *vc = avccontext->priv_data ;
    GetBitContext *gb = &(vc->gb);
    int hdr_type;
...
static int vorbis_decode_init(AVCodecContext *avccontext) {
    vorbis_context *vc = avccontext->priv_data ;
    const uint8_t *headers = avccontext->extradata;
    int headers_len=avccontext->extradata_size;
    int header_len[3];
    int i, j;

    avccontext->sample_fmt=SAMPLE_FMT_FLT;
    ...
    if (vorbis_init_id_header(avccontext, headers, header_len[0])<0)
        return -1;

    if (vorbis_init_setup_header(avccontext, headers+header_len[0]+header_len[1], header_len[2])<0)
        return -1;

    if (vorbis_init_comment_header(avccontext, headers+header_len[0],header_len[1])<0)
        return -1;
...
static uint_fast8_t vorbis_floor1_decode(vorbis_context *vc, vorbis_floor_data *vfu, float *vec) {
    ...
    GetBitContext *gb=&vc->gb;
    static const uint_fast16_t range_v[4]={ 256, 128, 86, 64 };
    ...
    uint_fast8_t *floor1_flag=_alloca(vf->x_list_dim*sizeof(uint_fast8_t));
    #endif
    uint_fast8_t class_;
    uint_fast8_t cdim;
    uint_fast8_t cbits;
                        ...
                        uint8_t *buf, int buf_size)
{
    ...
    if (!avccontext->extradata && *buf&PACKET_TYPE_HEADER) {
        if (vc->inited) {
            vorbis_free(avccontext->priv_data);
            memset(avccontext->priv_data,0,sizeof(*vc));
            vc->avccontext = avccontext;
        }
        switch (*buf&PACKET_TYPE_BITS) {
            case PACKET_TYPE_HEADER:
                if (vorbis_init_id_header(avccontext,buf,buf_size)<0)
                    return -1;
                break;
            case PACKET_TYPE_COMMENT:
                if (vorbis_init_comment_header(avccontext,buf,buf_size)<0)
                    return -1;
                break;
            case PACKET_TYPE_CODEBOOK:
                if (vorbis_init_setup_header(avccontext,buf,buf_size)<0)
                    return -1;
                break;
        }
        *data_size=0;
    ...
    if (avccontext->channels>2) {
        chm=chmap[avccontext->channels-1];
        for(i=0;i<len;i+=avccontext->channels) {
            int c;
            for (c=0;c<avccontext->channels;++c)
                ((float*)data)[i+c]=vc->ret[i+chm[c]];
        }
    }else{
        memcpy(data,vc->ret,sizeof(float)*len);
    }
    *data_size=len*sizeof(float);
