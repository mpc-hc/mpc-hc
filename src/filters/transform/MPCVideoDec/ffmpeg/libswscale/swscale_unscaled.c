...
static int planarToYuy2Wrapper(SwsContext *c, const uint8_t *src[],
                               ...
                               uint8_t *dstParam[], stride_t dstStride[])
{
...
static int palToRgbWrapper(SwsContext *c, const uint8_t *src[], stride_t srcStride[],
...
static void gbr24ptopacked24(const uint8_t* src[], stride_t srcStride[], uint8_t* dst, stride_t dstStride, int srcSliceH, int width)
{
    int x, h, i;
...
static void gbr24ptopacked32(const uint8_t* src[], stride_t srcStride[], uint8_t* dst, stride_t dstStride, int srcSliceH, int alpha_first, int width)
{
    int x, h, i;
    for (h = 0; h < srcSliceH; h++) {
        uint8_t *dest = dst + dstStride * h;
...
static int planarRgbToRgbWrapper(SwsContext *c, const uint8_t* src[], stride_t srcStride[], int srcSliceY,
...
int attribute_align_arg sws_scale(struct SwsContext *c,
                                  ...
                                  const stride_t srcStride[], int srcSliceY,
                                  ...
                                  const stride_t dstStride[])
{
    ...
    if (c->sliceDir == 1) {
        ...
        stride_t srcStride2[4] = { srcStride[0], srcStride[1], srcStride[2],
                              srcStride[3] };
        stride_t dstStride2[4] = { dstStride[0], dstStride[1], dstStride[2],
                              dstStride[3] };
