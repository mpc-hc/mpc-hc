...
CDirectVobSub::CDirectVobSub()
{
	...
	m_fSubtitlesOverlapping = !!theApp.GetProfileInt(ResStr(IDS_R_GENERAL), ResStr(IDS_RG_SUBTITLESOVERLAPPING), 1);
...
STDMETHODIMP CDirectVobSub::get_SubtitlesOverlapping(bool* fSubtitlesOverlapping)
{
    CAutoLock cAutoLock(&m_propsLock);

    return fSubtitlesOverlapping ? *fSubtitlesOverlapping = m_fSubtitlesOverlapping, S_OK : E_POINTER;
}

STDMETHODIMP CDirectVobSub::put_SubtitlesOverlapping(bool fSubtitlesOverlapping)
{
    CAutoLock cAutoLock(&m_propsLock);

    if (m_fSubtitlesOverlapping == fSubtitlesOverlapping) {
        return S_FALSE;
    }

    m_fSubtitlesOverlapping = fSubtitlesOverlapping;
...
STDMETHODIMP CDirectVobSub::UpdateRegistry()
{
	...
	theApp.WriteProfileInt(ResStr(IDS_R_GENERAL), ResStr(IDS_RG_SUBTITLESOVERLAPPING), m_fSubtitlesOverlapping);
