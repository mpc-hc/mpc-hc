...
CDVBChannel::CDVBChannel()
    ...
    , m_nVideo_fps(DVB_FPS_25_0)
    , m_nVideo_Chroma(DVB_Chroma_4_2_0)
    , m_nVideo_Width(0)
    , m_nVideo_Height(0)
    , m_nVideo_AR(DVB_AR_NULL)
    , m_nAudioCount(0)
    , m_nDefaultAudio(0)
    , m_nSubtitleCount(0)
...
void CDVBChannel::FromString(CString strValue)
{
    ...
    if (nVersion > FORMAT_VERSION_2) {
        ...
        m_nVideo_Height = _tstol(strValue.Tokenize(_T("|"), i));
        m_nVideo_AR = (DVB_AspectRatio_TYPE)_tstol(strValue.Tokenize(_T("|"), i));
    }
...
{
    CString strValue;
    strValue.AppendFormat(_T("%d|%s|%ld|%d|%d|%d|%d|%ld|%ld|%ld|%ld|%ld|%ld|%d|%d|%d|%ld|%ld|%d|%ld|%d|%ld|%d"),
