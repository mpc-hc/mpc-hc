...
SRESULT podnapisi::Search(const SubtitlesInfo& pFileInfo)
{
    ...
    do {
        ...
        if (dxml.Parse(data.c_str()) == tinyxml2::XMLError::XML_SUCCESS) {
            ...
            auto pRootElmt = dxml.FirstChildElement("results");
            if (pRootElmt) {
                auto pPaginationElmt = pRootElmt->FirstChildElement("pagination");
                if (pPaginationElmt) {
                ...
                if (results > 0) {
                    auto pSubtitleElmt = pRootElmt->FirstChildElement("subtitle");
