...
SRESULT OpenSubtitles::Search(const SubtitlesInfo& fileInfo, volatile BOOL& _bAborting)
{
    ...
    args[1][0]["moviebytesize"] = std::to_string(fileInfo.fileSize);
    //args[1][1]["sublanguageid"] = !languages.empty() ? languages : "all";
    //args[1][1]["tag"] = fileInfo.fileName + "." + fileInfo.fileExtension;
    args[2]["limit"] = 500;
        ...
        if (result["data"].getType() == XmlRpcValue::Type::TypeArray) {
            ...
            if (!args[1]["baseinfo"]["idmovieimdb"].valid()) {
                ...
                if (_result["data"].getType() == XmlRpcValue::Type::TypeStruct) {
                    ...
                    if (_stricmp(title.c_str(), _title.c_str()) == 0 /*&& (fileInfo.year == -1 || (fileInfo.year != -1 && fileInfo.year == atoi(_result["data"][fileInfo.fileHash]["MovieYear"])))*/) {
            ...
            if (!args[1]["baseinfo"]["idmovieimdb"].valid()) {
                ...
                if (_result["data"].getType() == XmlRpcValue::Type::TypeArray) {
                    ...
                    for (int i = 0; i < nCount; ++i) {
                        ...
                        if (_stricmp(title.c_str(), _title.c_str()) == 0 /*&& (fileInfo.year == -1 || (fileInfo.year != -1 && fileInfo.year == atoi(_result["data"][fileInfo.fileHash][i]["MovieYear"])))*/) {
                ...
                if (_result["data"].getType() == XmlRpcValue::Type::TypeArray) {
                    ...
                    for (int i = 0; i < nCount; ++i) {
                        ...
                        if (results.size() == 1) {
                            ...
                            if (_stricmp(title.c_str(), _title.c_str()) == 0 /*&& (fileInfo.year == -1 || (fileInfo.year != -1 && fileInfo.year == atoi(results[0][1].c_str())))*/) {
