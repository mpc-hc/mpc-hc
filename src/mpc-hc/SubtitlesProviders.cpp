...
void SubtitlesProviders::Search(BOOL bAutoDownload)
{
    ...
    if (CheckInternetConnection()) {
        ...
        pThread->Initialize(&m_MainFrame, this, SubtitlesThreadType(STT_SEARCH | (bAutoDownload ? STT_DOWNLOAD : NULL)));
...
void SubtitlesProviders::Download(SubtitlesInfo& fileInfo, BOOL bActivate)
{
    if (CheckInternetConnection()) {
        SubtitlesThread* pThread = (SubtitlesThread*)AfxBeginThread(RUNTIME_CLASS(SubtitlesThread));
        pThread->Initialize(&m_MainFrame, this, STT_DOWNLOAD);
...
void SubtitlesProviders::Upload()
{
    if (CheckInternetConnection()) {
        ...
        m_MainFrame.SendMessage(WM_GETSUBTITLES, 0, (LPARAM)&fileInfo);
        //fileInfo.GetCurrentSubtitles();

        if (!fileInfo.fileContents.empty()) {
            SubtitlesThread* pThread = (SubtitlesThread*)AfxBeginThread(RUNTIME_CLASS(SubtitlesThread));
            pThread->Initialize(&m_MainFrame, this, STT_UPLOAD);
...
{
    m_MainFrame->m_wndSubtitlesUploadDialog.DoFinished(m_bAbort);
}
