...
void CSimpleTextSubtitle::Copy(CSimpleTextSubtitle& sts)
{
    if (this != &sts) {
        ...
        m_fHearingImpaired = sts.m_fHearingImpaired;
...
bool CSimpleTextSubtitle::Open(CString provider, BYTE* data, int len, int CharSet, CString name, int fHearingImpaired, LCID lcid)
{
    bool fRet = Open(data, len, CharSet, name);

    m_provider = provider;
    m_fHearingImpaired = fHearingImpaired;
    m_lcid = lcid;
    return fRet;
}
