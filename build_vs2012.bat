...
IF /I "%Rebuild%" == "FFmpeg" CALL "src\thirdparty\ffmpeg\gccbuild.bat" Rebuild %PPLATFORM% %BUILDCFG% VS2012
...
MSBuild.exe mpc-hc_vs2012.sln %MSBUILD_SWITCHES%^
...
IF %ERRORLEVEL% NEQ 0 (
  CALL :SubMsg "ERROR" "mpc-hc_vs2012.sln %BUILDCFG% Filter %1 - Compilation failed!"
  EXIT /B
) ELSE (
  CALL :SubMsg "INFO" "mpc-hc_vs2012.sln %BUILDCFG% Filter %1 compiled successfully"
)
EXIT /B
...
IF %ERRORLEVEL% NEQ 0 (
  CALL :SubMsg "ERROR" "mpc-hc_vs2012.sln %BUILDCFG% %1 - Compilation failed!"
  EXIT /B
) ELSE (
  CALL :SubMsg "INFO" "mpc-hc_vs2012.sln %BUILDCFG% %1 compiled successfully"
)
EXIT /B
...
MSBuild.exe mpciconlib_vs2012.sln %MSBUILD_SWITCHES%^
 /target:%BUILDTYPE% /property:Configuration=Release;Platform=%1
IF %ERRORLEVEL% NEQ 0 (
  CALL :SubMsg "ERROR" "mpciconlib_vs2012.sln %1 - Compilation failed!"
  EXIT /B
) ELSE (
  CALL :SubMsg "INFO" "mpciconlib_vs2012.sln %1 compiled successfully"
)
EXIT /B
...
"%InnoSetupPath%" /Q /O"bin12" "distrib\mpc-hc_setup.iss" %MPCHC_INNO_DEF% /DVS2012
IF %ERRORLEVEL% NEQ 0 CALL :SubMsg "ERROR" "Compilation failed!" & EXIT /B
...
ECHO        Debug only applies to mpc-hc_vs2012.sln.
